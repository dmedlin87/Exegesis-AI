...
if Counter is not None:  # pragma: no cover - exercised when metrics available
    _LEDGER_STATE_TRANSITIONS = Counter(
        "EXEGESIS_router_ledger_state_transitions_total",
        "Observed deduplicated inflight state transitions.",
        labelnames=("status",),
    )
    _LEDGER_EVENTS = Counter(
        "EXEGESIS_router_ledger_events_total",
...
class SharedLedger:
    ...
    def __init__(self, path: str | None = None) -> None:
        default_path = Path(tempfile.gettempdir()) / "EXEGESIS_router_ledger.db"
        if path is not None:
            resolved_path = Path(path)
        else:
            env_path = os.environ.get("EXEGESIS_ROUTER_LEDGER_PATH")
