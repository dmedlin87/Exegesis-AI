...
from exegesis.application.core.telemetry import RAG_CACHE_EVENTS_METRIC
        ...
        if self._redis_module is None:  # pragma: no cover - optional dependency
            if allow_insecure_startup():
                LOGGER.warning(
                    ...
                    " disabled because EXEGESIS_ALLOW_INSECURE_STARTUP is enabled."
                )
                return None
            raise RuntimeError(
                "Redis support is required for RAG caching. Install the redis"
                " dependency or set EXEGESIS_ALLOW_INSECURE_STARTUP=1 for local"
                " testing."
            )
        ...
        try:
            ...
            if allow_insecure_startup():
                ...
                raise RuntimeError(
                    ...
                    " and credentials or enable EXEGESIS_ALLOW_INSECURE_STARTUP=1 for"
                    " local testing."
                ) from exc
        return self._client
...
def extract_cache_key_suffix(cache_key: str | None) -> str | None:
    """Return the identifying suffix for a cache key."""
