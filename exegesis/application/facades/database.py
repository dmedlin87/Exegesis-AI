...
try:  # pragma: no cover - optional sqlite context cleanup shim
    ...
    if not getattr(sqlite3, "__EXEGESIS_closing_patch__", False):
        ...
        sqlite3.__EXEGESIS_closing_patch__ = True  # type: ignore[attr-defined]

if os.name == "nt" and not getattr(Path, "__EXEGESIS_unlink_retry__", False):
    ...
    Path.__EXEGESIS_unlink_retry__ = True  # type: ignore[attr-defined]
...
def _create_engine(database_url: str) -> Engine:
    ...
    if database_url.startswith("sqlite") and not getattr(
        engine, "__EXEGESIS_dispose_wrapped__", False
    ):
        ...
        engine.__EXEGESIS_dispose_wrapped__ = True  # type: ignore[attr-defined]
    return engine
