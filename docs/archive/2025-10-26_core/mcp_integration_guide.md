...
# Exegesis AI MCP Integration Execution Guide

This guide turns the high-level MCP integration plan into a concrete runbook for operators and developers. It covers the full lifecycle from standing up the Exegesis AI stack through validating the exposed Model Context Protocol tools and documenting the connector for ChatGPT.

---

## 1. Provision Exegesis AI services with required credentials
...
1. Copy `infra/.env.example` to a working file (for example `infra/.env.local`). Fill in:
   ...
   - `EXEGESIS_STORAGE_PATH` with a persistent folder mounted into the API/worker containers.
   - Embedding model configuration (`EMBEDDING_MODEL`, `EMBEDDING_API_BASE`, `EMBEDDING_API_KEY`), or fall back to the default BGE-M3 settings packaged with Exegesis AI for local experiments.
2. Configure API auth before launching services:
   - Provide at least one API key via `EXEGESIS_API_KEYS="local-dev-key"`, or set `EXEGESIS_AUTH_ALLOW_ANONYMOUS=1` when running isolated local smoke tests.
   - If JWT auth is required, set `EXEGESIS_AUTH_JWT_SECRET` and corresponding issuer/audience values.
...
2. Set `MCP_SCHEMA_BASE_URL` to the externally reachable base URL of the MCP deployment (for example `https://Exegesis AI.example.com/mcp`). This value is used to build `$id` and `$ref` fields inside `/metadata` responses so ChatGPT can dereference schemas.
...
## 3. Seed Exegesis AI with representative content
...
2. Run the ingestion CLI from the repository root (it connects directly to the
   ...
   ```
   Set any required environment variables (for example `EXEGESIS_API_KEYS`,
...
1. Draft operator documentation (for example under `docs/connector/README.md`) outlining how to register the Exegesis AI connector inside ChatGPT developer mode. Include:
...
Following this runbook will produce a fully operational Exegesis AI MCP deployment that ChatGPT can use for hybrid search, verse aggregation, timeline analysis, and research note workflows.
