...
from exegesis.services.cli.ingest_folder import ingest_folder
...
def test_dry_run_lists_supported_files(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.run_pipeline_for_file", _fail_if_called
    )
...
def test_api_mode_processes_batches_with_overrides(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.run_pipeline_for_file", _fake_run_pipeline
    )
...
def test_worker_mode_enqueues_tasks(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.worker_tasks.process_file",
        _FakeProcessFile(),
    )
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.run_pipeline_for_file", _unexpected
    )
...
def test_post_batch_tags_runs_enricher(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.MetadataEnricher", _FakeEnricher
    )
...
def test_post_batch_multiple_steps_dispatches_followups(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.worker_tasks.enrich_document",
        _FakeEnrichTask(),
    )
    monkeypatch.setattr("exegesis.services.cli.ingest_folder.httpx.Client", _FakeClient)
...
def test_post_batch_skipped_for_worker_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.run_pipeline_for_file",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("API mode should not run")
        ),
    )
    monkeypatch.setattr(
        "exegesis.services.cli.ingest_folder.MetadataEnricher",
