...
from exegesis.adapters.persistence.models import Discovery, Document
...
from exegesis.infrastructure.api.app.infra.ingestion_service import get_ingestion_service
...
def _use_real_discovery_service() -> Generator[None, None, None]:
    ...
    for module_name in (
        "exegesis.infrastructure.api.app.research.discoveries",
        "exegesis.infrastructure.api.app.research.discoveries.tasks",
    ):
        stubbed_modules[module_name] = sys.modules.pop(module_name, None)

    discoveries_module = importlib.import_module("exegesis.infrastructure.api.app.research.discoveries")
    real_service = discoveries_module.DiscoveryService
    routes_module = importlib.import_module("exegesis.infrastructure.api.app.routes.discoveries")
...
def schedule_refresh_mock(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr(
        "exegesis.infrastructure.api.app.routes.ingest.schedule_discovery_refresh",
