...
from exegesis.application.facades.research import set_application_resolver
from exegesis.application.facades.resilience import set_resilience_policy_factory
from exegesis.application.facades.runtime import (
    allow_insecure_startup,
    current_runtime_environment,
    generate_ephemeral_dev_key,
)
from exegesis.application.facades.secret_migration import set_llm_registry_saver
from exegesis.application.facades.security import (
    set_principal_resolver as _set_principal_resolver,
)
from exegesis.application.facades.settings import (
    Settings,
    get_settings,
    get_settings_secret,
)
...
def register_trace_handlers(app: FastAPI) -> None:
    ...
    async def EXEGESIS_error_with_trace(request, exc: TheoError):  # pragma: no cover - thin wrapper
        trace_context = get_trace_context()
        trace_id = getattr(trace_context, "trace_id", None)
