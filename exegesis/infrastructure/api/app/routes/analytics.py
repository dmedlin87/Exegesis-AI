...
def ingest_client_telemetry(payload: TelemetryBatch) -> dict[str, str]:
    ...
    if not payload.events:
        LOGGER.warning(
            "Received telemetry payload without events; no records created.",
            extra={"page": payload.page},
        )
...
def ingest_feedback_event(
    ...
    try:
        ...
        session.rollback()
        LOGGER.error("Failed to persist feedback event to the database", exc_info=db_err)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Temporary database error while persisting feedback.",
        ) from db_err
