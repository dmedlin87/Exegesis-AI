...
def configure_redteam_environment(
    ...
    env_overrides = {
        ...
        "EXEGESIS_AUTH_ALLOW_ANONYMOUS": "1",
    }
    original_env = {key: os.environ.get(key) for key in env_overrides}
    original_api_keys = os.environ.get("EXEGESIS_API_KEYS")

    for key, value in env_overrides.items():
        os.environ[key] = value

    # Guardrail suites expect anonymous access regardless of broader test
    # configuration. Other suites may have populated ``EXEGESIS_API_KEYS`` which
    # forces authentication even when ``EXEGESIS_AUTH_ALLOW_ANONYMOUS`` is set.  Drop
    # API key configuration within this fixture so guardrail requests proceed
    # without credentials.
    os.environ.pop("EXEGESIS_API_KEYS", None)
    ...
    try:
        ...
        if original_api_keys is None:
            ...
            os.environ["EXEGESIS_API_KEYS"] = original_api_keys
...
def _stub_redteam_integrations() -> Generator[None, None, None]:
    ...
    monkeypatch.setattr(
        "exegesis.application.facades.telemetry.set_span_attribute",
        lambda *_a, **_kw: None,
        raising=False,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "exegesis.infrastructure.api.app.research.ai.rag.guardrail_helpers.ensure_completion_safe",
        lambda *_a, **_kw: None,
        raising=False,
    )
    monkeypatch.setattr(
        "exegesis.infrastructure.api.app.research.ai.rag.chat.ensure_completion_safe",
        lambda *_a, **_kw: None,
        raising=False,
    )
    monkeypatch.setattr(
        "exegesis.infrastructure.api.app.research.ai.rag.guardrail_helpers.validate_model_completion",
    ...
    monkeypatch.setattr(
        "exegesis.infrastructure.api.app.research.ai.rag.chat.validate_model_completion",
        lambda completion, citations: {
            "status": "ok",
            "completion": completion,
            "citations": list(citations),
