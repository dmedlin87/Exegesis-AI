...
def test_require_principal_allows_anonymous_when_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("EXEGESIS_AUTH_JWT_SECRET", "")
...
def test_require_principal_with_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("EXEGESIS_API_KEYS", f'["{api_key}"]')
    monkeypatch.setenv("EXEGESIS_AUTH_ALLOW_ANONYMOUS", "false")
    monkeypatch.setenv("EXEGESIS_AUTH_JWT_SECRET", "")
    _refresh_settings()
...
def test_require_principal_with_jwt(monkeypatch: pytest.MonkeyPatch) -> None:
    secret = "jwt-secret"
    monkeypatch.setenv("EXEGESIS_API_KEYS", "[]")
    monkeypatch.setenv("EXEGESIS_AUTH_JWT_SECRET", secret)
    monkeypatch.setenv("EXEGESIS_AUTH_ALLOW_ANONYMOUS", "false")
    _refresh_settings()
