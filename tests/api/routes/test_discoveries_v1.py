...
def client():
    """Module-scoped API client for tests."""
    with TestClient(app) as test_client:
        yield test_client


@pytest.fixture
def client_with_mock_repo(client, mock_discovery_repo):
    """Override discovery repository dependency for each test."""
    app.dependency_overrides[
        discoveries_v1.get_discovery_repository
    ] = lambda: mock_discovery_repo
    try:
        yield client
    finally:
        app.dependency_overrides.pop(
            discoveries_v1.get_discovery_repository, None
        )
...
class TestDiscoveriesV1Routes:
    ...
    def test_list_discoveries_with_filters(
        self, client_with_mock_repo, mock_discovery_repo
    ):
        """List discoveries applies all filters."""
        mock_discovery_repo.list.return_value = []

        response = client_with_mock_repo.get(
    ...
    def test_get_discovery_success(
        self, client_with_mock_repo, mock_discovery_repo, sample_discovery_dto
    ):
        """Get single discovery by ID."""
        mock_discovery_repo.get_by_id.return_value = sample_discovery_dto

        response = client_with_mock_repo.get("/discoveries/1?user_id=test_user")
    ...
    def test_get_discovery_not_found(self, client_with_mock_repo, mock_discovery_repo):
        """Get discovery returns 404 for missing discovery."""
        mock_discovery_repo.get_by_id.return_value = None

        response = client_with_mock_repo.get("/discoveries/999?user_id=test_user")
    ...
    def test_mark_viewed_not_found(self, client_with_mock_repo, mock_discovery_repo):
        """Mark viewed returns 404 when discovery doesn't exist."""
        mock_discovery_repo.mark_viewed.side_effect = LookupError("Not found")

        response = client_with_mock_repo.post("/discoveries/999/view?user_id=test_user")
