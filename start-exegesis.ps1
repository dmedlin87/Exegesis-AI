<#
.SYNOPSIS
    Intelligent launcher for Exegesis AI - ensures API and Web services run reliably.
...
.PARAMETER LogToFile
    Writes logs to file in addition to console (logs/Exegesis AI-launcher.log)
...
.NOTES
    Exegesis AI Engine - Intelligent Service Launcher
...
$script:LogFile = Join-Path $script:LogsDir "Exegesis AI-launcher.log"
...
$script:Profiles = @{
    ...
    staging = @{
        ...
        WebPort = 3100
        Env = @{
            EXEGESIS_ENVIRONMENT = 'staging'
            EXEGESIS_PROFILE = 'staging'
        }
    }
}
...
$script:CurrentProfile.Env.EXEGESIS_API_PORT = "$ApiPort"
$script:CurrentProfile.Env.EXEGESIS_WEB_PORT = "$WebPort"
...
if ((Get-Command Import-ServiceConfiguration -ErrorAction SilentlyContinue) -and (Test-Path $ConfigPath)) {
    try {
        ...
        Write-Exegesis AILog "Warning: Could not load service configuration: $_" -Level Warning
...
if (Get-Command Register-MetricsEndpoint -ErrorAction SilentlyContinue) {
    try {
        ...
        if ($configMetricsPort -gt 0) {
            Write-Exegesis AILog "Metrics endpoint available at http://localhost:$configMetricsPort/metrics" -Level Info
        }
        if ($configMetricsFile) {
            Write-Exegesis AILog "Metrics snapshot file: $configMetricsFile" -Level Debug
        }
    } catch {
        Write-Exegesis AILog "Warning: Could not register metrics endpoint: $_" -Level Debug
    }
}
...
function Write-Exegesis AIBanner {
    Write-Host "`n" -NoNewline
    Write-Host "===========================================================" -ForegroundColor Magenta
    Write-Host "|                                                         |" -ForegroundColor Magenta
    Write-Host "|              " -ForegroundColor Magenta -NoNewline
    Write-Host "Exegesis AI ENGINE" -ForegroundColor White -NoNewline
...
function Invoke-LauncherHelper {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Exegesis AILog "Launcher helper exited with code $LASTEXITCODE" -Level Debug -Metadata @{ args = $Arguments }
            return $null
        }
        return $output
    } catch {
        Write-Exegesis AILog "Failed to invoke launcher helper: $_" -Level Debug
        return $null
    }
}
...
function Get-PrerequisiteReport {
    ...
    try {
        ...
        Write-Exegesis AILog "Unable to parse prerequisite report: $_" -Level Debug
        return $null
    }
}
...
function Invoke-RuntimeInstallation {
    ...
    foreach ($installer in $Runtime.installers) {
        ...
        Write-Exegesis AILog "Attempting to install $($Runtime.name) using $command..." -Level Info

        try {
            $process = Start-Process -FilePath $command -ArgumentList $installArgs -Wait -NoNewWindow -PassThru -ErrorAction Stop
            if ($process.ExitCode -eq 0) {
                Write-Exegesis AILog "$($Runtime.name) installation completed" -Level Success
                return $true
            }
        } catch {
            Write-Exegesis AILog "Automatic installation failed: $_" -Level Warning
        }
    }
...
function Invoke-DockerFallback {
    ...
    if (-not $ComposeCommand) {
        Write-Exegesis AILog "Docker Compose is not available on this system" -Level Error
        return
    }

    Write-Exegesis AILog "Launching Docker Compose fallback (this will replace local processes)..." -Level Warning

    $env:EXEGESIS_PROFILE = $script:ProfileName
    $env:EXEGESIS_API_PORT = "$ApiPort"
    $env:EXEGESIS_WEB_PORT = "$WebPort"
    $env:EXEGESIS_USE_HTTPS = if ($script:HttpsEnabled) { '1' } else { '0' }
    ...
    try {
        ...
        $env:EXEGESIS_PROFILE = $null
        $env:EXEGESIS_API_PORT = $null
        $env:EXEGESIS_WEB_PORT = $null
        $env:EXEGESIS_USE_HTTPS = $null
    }
}
...
function Initialize-DevCertificates {
    ...
    if (-not $result) {
        Write-Exegesis AILog "Failed to generate development certificates. HTTPS mode will be disabled." -Level Warning
    ...
    try {
        ...
        Write-Exegesis AILog "Development certificates ready at $($certInfo.cert_path)" -Level Success
    } catch {
        Write-Exegesis AILog "Could not parse certificate metadata: $_" -Level Warning
        $script:Certificates.Enabled = $false
        $script:HttpsEnabled = $false
    }
}
...
function Initialize-Telemetry {
    ...
    if (-not $consent) {
        ...
        Write-Exegesis AILog "Telemetry is disabled by default. Run with -TelemetryOptIn to contribute anonymous launcher metrics." -Level Info
    } elseif ($TelemetryOptIn) {
        Write-Exegesis AILog "Telemetry opt-in recorded. Thank you for helping improve Exegesis AI!" -Level Success
    } elseif ($TelemetryOptOut) {
        Write-Exegesis AILog "Telemetry has been disabled." -Level Warning
    }
    ...
    try {
        ...
        Write-Exegesis AILog "Unable to persist telemetry preference: $_" -Level Debug
    }
}
...
function Write-TelemetryEvent {
    ...
    try {
        ...
        Write-Exegesis AILog "Failed to write telemetry event: $_" -Level Debug
    }
}
...
function New-ServiceEnvironment {
    ...
    $envClone['EXEGESIS_USE_HTTPS'] = if ($script:HttpsEnabled) { '1' } else { '0' }
    $envClone['EXEGESIS_PROFILE'] = $script:ProfileName
    $envClone['EXEGESIS_API_PORT'] = "$ApiPort"
    $envClone['EXEGESIS_WEB_PORT'] = "$WebPort"
...
function Test-Prerequisite {
    ...
    Write-Exegesis AILog "Checking $Name..." -Level Debug
    
    try {
        $result = & $Command $VersionArg 2>&1
        if ($LASTEXITCODE -eq 0) {
            $version = if ($result -is [array]) { $result[0].ToString().Trim() } else { $result.ToString().Trim() }
            Write-Exegesis AILog "$Name detected: $version" -Level Success
            return $true
        }
    } catch {
        Write-Exegesis AILog "$Name not found: $_" -Level Error
        return $false
    }
    
    Write-Exegesis AILog "$Name not found or invalid" -Level Error
    return $false
}
...
function Test-PortAvailable {
    ...
    if ($Port -le 1023) {
        Write-Exegesis AILog "Warning: Port $Port is in the well-known port range (1-1023) and may require elevated privileges" -Level Warning
    }
    ...
    try {
        ...
        Write-Exegesis AILog "Port $Port is in use or unavailable: $_" -Level Debug
        return $false
    }
}
...
function Stop-Exegesis AIService {
    param(
        [Parameter(Mandatory=$true)]
        [ValidateSet('Api', 'Web')]
        [string]$ServiceKey,
    ...
    if (-not $script:Services.ContainsKey($ServiceKey)) {
        Write-Exegesis AILog "Unknown service key: $ServiceKey" -Level Warning
        return
    }
    
    $service = $script:Services[$ServiceKey]
    if (-not $service -or -not $service.Job) { 
        Write-Exegesis AILog "Service $ServiceKey has no active job" -Level Debug
        return 
    }
    
    Write-Exegesis AILog "Stopping $($service.Name)..." -Level Info
    ...
    try {
        ...
        if ($Verbose -and $output) {
            Write-Exegesis AILog "Service output:" -Level Debug
            $output | ForEach-Object { Write-Exegesis AILog "  $_" -Level Debug }
        }
        ...
        if ($service.StartTime) {
            ...
            Write-Exegesis AILog "$($service.Name) stopped (uptime: $($uptime.ToString('hh\:mm\:ss')))" -Level Success -Metadata @{ uptime_seconds = $uptime.TotalSeconds }
            $service.StartTime = $null
        } else {
            Write-Exegesis AILog "$($service.Name) stopped" -Level Success
        }
    } catch {
        Write-Exegesis AILog "Error stopping $($service.Name): $_" -Level Error -Metadata @{ error = $_.Exception.Message }
    }
...
function Wait-ServiceReady {
    ...
    Write-Exegesis AILog "Waiting for $($script:Services[$ServiceKey].Name) to become ready..." -Level Debug
    $stopwatch = [System.Diagnostics.Stopwatch]::StartNew()
    ...
    while ($stopwatch.Elapsed.TotalSeconds -lt $MaxWaitSeconds) {
        ...
        if ($Job.State -eq "Failed" -or $Job.State -eq "Stopped") {
            ...
            Write-Exegesis AILog "$($script:Services[$ServiceKey].Name) failed to start:" -Level Error -Metadata @{ error = $errorOutput }
            Write-Exegesis AILog $errorOutput -Level Error
        ...
        if (Test-ServiceHealth -Endpoint $HealthEndpoint -Diagnostics ([ref]$diagnostics)) {
            ...
            Write-Exegesis AILog "$($script:Services[$ServiceKey].Name) is ready at $DisplayUrl (startup time: ${startupTime}s)" -Level Success -Metadata @{ startup_time_seconds = $startupTime; response_time_ms = $diagnostics.ResponseTime }
            return $true
        }
    ...
    if ($jobOutput.Trim()) {
        Write-Exegesis AILog "$($script:Services[$ServiceKey].Name) output during timeout:" -Level Warning
        Write-Exegesis AILog $jobOutput -Level Error
    }
    
    $script:Services[$ServiceKey].LastError = "Timeout after ${MaxWaitSeconds}s"
    Write-Exegesis AILog "$($script:Services[$ServiceKey].Name) did not become healthy within ${MaxWaitSeconds}s" -Level Warning -Metadata @{ timeout_seconds = $MaxWaitSeconds; job_state = $Job.State }
    Write-Exegesis AILog "Job state: $($Job.State) - Check output above for errors" -Level Error
    $script:Services[$ServiceKey].StartTime = $null
    return $false
}

function Start-Exegesis AIApi {
    Write-Exegesis AILog "Starting Exegesis AI API on port $ApiPort..." -Level Info

    # Check port availability
    if (-not (Test-PortAvailable -Port $ApiPort)) {
        Write-Exegesis AILog "Port $ApiPort is already in use" -Level Error
        return $false
    }
    ...
    if (-not (Test-Path $venvPython)) {
        Write-Exegesis AILog "Virtual environment not found at $venvPython" -Level Error
        Write-Exegesis AILog "Please create a virtual environment first: python -m venv .venv" -Level Error
        return $false
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Exegesis AILog "Uvicorn not found in virtual environment" -Level Error
            Write-Exegesis AILog "Please install dependencies: .venv\Scripts\pip install .[api] -c constraints\\api.txt; .venv\\Scripts\\pip install .[ml] -c constraints\\ml.txt; .venv\\Scripts\\pip install .[dev] -c constraints\\dev.txt" -Level Error
            return $false
        }
    } catch {
        Write-Exegesis AILog "Failed to verify uvicorn: $_" -Level Error
        return $false
    }
    ...
    $truthy = @('1', 'true', 'yes')
    $localOverrideFlag = [System.Environment]::GetEnvironmentVariable('EXEGESIS_LOCAL_INSECURE_OVERRIDES')
    $enableLocalInsecure = $script:ProfileName -in @('dev', 'local')

    if ($localOverrideFlag -eq '0') {
        $enableLocalInsecure = $false
        Write-Exegesis AILog "Skipping insecure overrides (EXEGESIS_LOCAL_INSECURE_OVERRIDES=0)" -Level Info
    }

    if ($enableLocalInsecure) {
        $existingInsecure = if ($apiEnv.ContainsKey('EXEGESIS_ALLOW_INSECURE_STARTUP')) { $apiEnv['EXEGESIS_ALLOW_INSECURE_STARTUP'] } else { $null }
        ...
        if (-not $truthy.Contains($existingInsecureText)) {
            $apiEnv['EXEGESIS_ALLOW_INSECURE_STARTUP'] = '1'
            Write-Exegesis AILog "Enabling EXEGESIS_ALLOW_INSECURE_STARTUP=1 for local development" -Level Warning
        }
        $existingAnonymous = if ($apiEnv.ContainsKey('EXEGESIS_AUTH_ALLOW_ANONYMOUS')) { $apiEnv['EXEGESIS_AUTH_ALLOW_ANONYMOUS'] } else { $null }
        ...
        if (-not $truthy.Contains($existingAnonymousText)) {
            $apiEnv['EXEGESIS_AUTH_ALLOW_ANONYMOUS'] = '1'
            Write-Exegesis AILog "Enabling EXEGESIS_AUTH_ALLOW_ANONYMOUS=1 for local development" -Level Warning
        }
    } else {
        foreach ($key in @('EXEGESIS_ALLOW_INSECURE_STARTUP', 'EXEGESIS_AUTH_ALLOW_ANONYMOUS')) {
            if ($apiEnv.ContainsKey($key)) {
                ...
                if ($truthy.Contains($currentText)) {
                    $apiEnv[$key] = '0'
                    Write-Exegesis AILog "Clearing $key for non-local profile" -Level Info
                }
            }
        }
    }
    $apiEnv['PYTHONUNBUFFERED'] = '1'
    if (-not $apiEnv.ContainsKey('EXEGESIS_FORCE_EMBEDDING_FALLBACK')) {
        $fallbackSetting = [System.Environment]::GetEnvironmentVariable('EXEGESIS_FORCE_EMBEDDING_FALLBACK')
        if (-not [string]::IsNullOrWhiteSpace($fallbackSetting)) {
            $apiEnv['EXEGESIS_FORCE_EMBEDDING_FALLBACK'] = $fallbackSetting
            ...
            $apiEnv['EXEGESIS_FORCE_EMBEDDING_FALLBACK'] = '1'
        }
    }
    if (-not $apiEnv.ContainsKey('redis_url')) {
        ...
        if ($shouldDisableRedis) {
            $apiEnv['redis_url'] = ''
            Write-Exegesis AILog "Redis broker not detected locally; skipping Redis-dependent health probes" -Level Warning
        }
    }

    $httpsEnabled = $script:HttpsEnabled
    if ($httpsEnabled) {
        $apiEnv['EXEGESIS_CERT_PATH'] = $script:Certificates.CertPath
        $apiEnv['EXEGESIS_KEY_PATH'] = $script:Certificates.KeyPath
    } elseif ($script:HttpsRequested) {
        Write-Exegesis AILog "HTTPS requested but certificates were not generated. Falling back to HTTP." -Level Warning
    ...
    try {
        ...
        if ($success) {
            ...
            Write-Exegesis AILog "API Docs: $docsUrl" -Level Info
        }
        ...
        Write-Exegesis AILog "Failed to start API: $_" -Level Error -Metadata @{ error = $_.Exception.Message }
        $script:Services.Api.StartTime = $null
        return $false
    }
}

function Start-Exegesis AIWeb {
    Write-Exegesis AILog "Starting Exegesis AI Web UI on port $WebPort..." -Level Info

    # Check port availability
    if (-not (Test-PortAvailable -Port $WebPort)) {
        Write-Exegesis AILog "Port $WebPort is already in use" -Level Error
        return $false
    }

    # Verify web root exists
    if (-not (Test-Path $script:WebRoot)) {
        Write-Exegesis AILog "Web root not found at $script:WebRoot" -Level Error
        return $false
    }
    ...
    if (-not (Test-Path $packageJson)) {
        Write-Exegesis AILog "package.json not found at $packageJson" -Level Error
        return $false
    }
    ...
    if (-not (Test-Path $nodeModulesPath)) {
        Write-Exegesis AILog "Installing Node.js dependencies (first time only)..." -Level Info
        Push-Location $script:WebRoot
        try {
            $installOutput = npm install 2>&1
            if ($LASTEXITCODE -eq 0) {
                Write-Exegesis AILog "Dependencies installed successfully" -Level Success
            } else {
                Write-Exegesis AILog "npm install completed with exit code $LASTEXITCODE" -Level Warning
                if ($Verbose) {
                    Write-Exegesis AILog "npm output: $installOutput" -Level Debug
                }
            }
        } catch {
            Write-Exegesis AILog "Failed to install dependencies: $_" -Level Error
            Pop-Location
            return $false
        } finally {
            Pop-Location
    ...
    $httpsEnabled = $script:HttpsEnabled
    if ($httpsEnabled) {
        $webEnv['EXEGESIS_CERT_PATH'] = $script:Certificates.CertPath
        $webEnv['EXEGESIS_KEY_PATH'] = $script:Certificates.KeyPath
    ...
    $scriptBlock = {
        ...
        foreach ($key in @('EXEGESIS_AUTH_ALLOW_ANONYMOUS', 'EXEGESIS_ALLOW_INSECURE_STARTUP')) {
    ...
    try {
        ...
        Write-Exegesis AILog "Failed to start Web UI: $_" -Level Error -Metadata @{ error = $_.Exception.Message }
...
function Invoke-ServiceRestart {
    ...
    Stop-Exegesis AIService -ServiceKey $ServiceKey
    Start-Sleep -Seconds 2
    
    $success = switch ($ServiceKey) {
        "Api" { Start-Exegesis AIApi }
        "Web" { Start-Exegesis AIWeb }
        default { $false }
    }
    ...
    if ($success) {
        ...
        Write-Exegesis AILog "$($service.Name) restarted successfully" -Level Success -Metadata @{ restart_count = $service.RestartCount }
    } else {
        Write-Exegesis AILog "$($service.Name) restart failed" -Level Error
    }
...
function Start-HealthMonitor {
    if ($SkipHealthChecks) {
        Write-Exegesis AILog "Health monitoring disabled" -Level Warning
        return
    }

    $intervalSummary = ($script:Services.Keys | ForEach-Object { "$($_): $($script:Services[$_].HealthCheckInterval)s" }) -join ", "
    Write-Exegesis AILog "Starting health monitor (per-service intervals: $intervalSummary)" -Level Info
        ...
        foreach ($serviceKey in $script:Services.Keys) {
            ...
            if ($script:ProfilingEnabled -and (Get-Command Get-ProfilingSnapshot -ErrorAction SilentlyContinue)) {
                try {
                    ...
                    Write-Exegesis AILog "Failed to get profiling snapshot: $_" -Level Debug
                }
            }
            ...
            if (-not $isHealthy) {
                ...
                Write-Exegesis AILog "$($service.Name) health check failed: $($diagnostics.Error)" -Level Warning -Metadata @{
                    service = $serviceKey
                ...
                if ($script:AlertsEnabled -and $service.HealthCheckFailures -eq 1 -and (Get-Command Send-ServiceAlert -ErrorAction SilentlyContinue)) {
                    try {
                        ...
                        Write-Exegesis AILog "Failed to send service alert: $_" -Level Debug
                    }
                }
                ...
                if ($service.RestartCount -lt $script:MaxRestartAttempts) {
                    ...
                    if ($timeSinceLastRestart.TotalSeconds -gt $currentCooldown) {
                        Write-Exegesis AILog "Attempting to restart $($service.Name) (attempt $($service.RestartCount + 1)/$script:MaxRestartAttempts, cooldown: ${currentCooldown}s)..." -Level Warning -Metadata @{
                        ...
                        Write-Exegesis AILog "$($service.Name) in cooldown period (${remainingCooldown}s remaining)" -Level Debug
                    }
                } else {
                    Write-Exegesis AILog "$($service.Name) has exceeded maximum restart attempts ($script:MaxRestartAttempts)" -Level Error
                    Write-Exegesis AILog "Manual intervention required - check logs for details" -Level Error
                }
            } else {
                if ($service.HealthCheckFailures -gt 0) {
                    Write-Exegesis AILog "$($service.Name) recovered (response: $($diagnostics.ResponseTime)ms)" -Level Success
                    if ($script:AlertsEnabled -and (Get-Command Send-ServiceAlert -ErrorAction SilentlyContinue)) {
                        try {
                            ...
                            Write-Exegesis AILog "Failed to send recovery alert: $_" -Level Debug
                        }
                    }
                }

                $service.HealthCheckFailures = 0
                Write-Exegesis AILog "$($service.Name) health check passed" -Level Debug -Metadata @{
                    response_time_ms = $diagnostics.ResponseTime
            ...
            if (Get-Command Update-ServiceMetricsEntry -ErrorAction SilentlyContinue) {
                try {
                    ...
                    Write-Exegesis AILog "Failed to update metrics: $_" -Level Debug
                }
            }
...
function Resolve-Prerequisites {
    ...
    if ($report -and $report.runtimes) {
        foreach ($runtime in $report.runtimes) {
            ...
            Write-Exegesis AILog $message -Level $level
    ...
    foreach ($runtime in $missingLocal) {
        Write-Exegesis AILog "$($runtime.name) is required." -Level Warning
        if ($runtime.guidance) {
            Write-Exegesis AILog $runtime.guidance -Level Info
        }
    }
    ...
    if ($availableManagers.Count -gt 0) {
        foreach ($runtime in $missingLocal) {
            ...
            if ($promptResponse -and $promptResponse.Trim().ToLower() -in @('y', 'yes')) {
                ...
                if ($installed) {
                    Write-Exegesis AILog "Re-checking $($runtime.name) availability..." -Level Info
                }
            } else {
                Write-Exegesis AILog "Skipped automatic installation for $($runtime.name)." -Level Warning
            }
        }
    } else {
        Write-Exegesis AILog "No supported package manager (winget/choco/brew) detected for automatic installation." -Level Warning
    }
    ...
    if ($composeCommand) {
        Write-Exegesis AILog "Local prerequisites missing; attempting Docker Compose fallback." -Level Warning
    ...
    Write-Exegesis AILog "Cannot proceed without required runtimes. Install the missing tools listed above and try again." -Level Error
    return 'Failed'
}
...
function Start-Exegesis AI {
    Write-Exegesis AIBanner

    Write-Exegesis AILog "Initializing Exegesis AI Engine..." -Level Info
    Write-Exegesis AILog "Project Root: $script:ProjectRoot" -Level Debug

    if ($LogToFile) {
        Write-Exegesis AILog "Logging to file: $script:LogFile" -Level Info
    }
    if ($ShowMetrics) {
        Write-Exegesis AILog "Real-time metrics enabled (status dashboard every 60s)" -Level Info
    }
    ...
    if ($prereqResult -eq 'Failed') {
        Write-Exegesis AILog "Prerequisite resolution failed" -Level Error
...
public class Exegesis AIBypassCerts {
...
"@
            ...
            try {
                [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {[Exegesis AIBypassCerts]::Ignore}
            } catch {}
        }
    }
    ...
    if (-not (Test-Path $envPath)) {
        Write-Exegesis AILog ".env file not found, creating from template..." -Level Warning
        $envExample = Join-Path $script:ProjectRoot ".env.example"
        if (Test-Path $envExample) {
            Copy-Item $envExample $envPath
            Write-Exegesis AILog ".env file created successfully" -Level Success
        } else {
            Write-Exegesis AILog ".env.example not found, cannot create .env" -Level Error
    ...
    # Start API first
    $apiStarted = Start-Exegesis AIApi
    if (-not $apiStarted) {
        Write-Exegesis AILog "Failed to start API, aborting" -Level Error
        Write-TelemetryEvent -EventName 'launcher_failed' -Metadata @{ stage = 'api'; profile = $script:ProfileName }
        return
    }

    # Start Web UI
    $webStarted = Start-Exegesis AIWeb
    if (-not $webStarted) {
        Write-Exegesis AILog "Failed to start Web UI, stopping API" -Level Error
        Stop-Exegesis AIService -ServiceKey "Api"
    ...
    Write-Host "Exegesis AI IS READY!" -ForegroundColor White -NoNewline
    Write-Host "                        |" -ForegroundColor Green
    Write-Host "|                                                         |" -ForegroundColor Green
    ...
    $script:ExitHandler = Register-EngineEvent -SourceIdentifier PowerShell.Exiting -Action {
        Write-Host "`n`nShutting down Exegesis AI services..." -ForegroundColor Yellow
        try {
            Stop-Exegesis AIService -ServiceKey "Web" -ErrorAction SilentlyContinue
            Stop-Exegesis AIService -ServiceKey "Api" -ErrorAction SilentlyContinue
        } catch {
    ...
    try {
        ...
        if (Get-Command Stop-MetricsEndpoint -ErrorAction SilentlyContinue) {
            try {
                ...
                Write-Exegesis AILog "Failed to stop metrics endpoint: $_" -Level Debug
            }
        }

        # Graceful shutdown with timeout
        Stop-Exegesis AIService -ServiceKey "Web"
        Stop-Exegesis AIService -ServiceKey "Api"
        ...
        if ($LogToFile) {
            Write-Exegesis AILog "Session logs saved to: $script:LogFile" -Level Info
        }
        
        Write-Exegesis AILog "All services stopped. Goodbye!" -Level Success
    }
}
